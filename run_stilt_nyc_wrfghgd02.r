...
# User inputs ------------------------------------------------------------------
project <- 'halo'
stilt_wd <- file.path('/scratch/07351/tg866507/', project)
...
output_wd <- file.path(stilt_wd,'out_wrfghg_d02',paste0(substr(opt$file,13,nchar(opt$file)-4),'_',format(opt$alt,nsmall=1)))
...
run_time_str <- strftime(receptors$run_time,'%Y%m%d%H%M%S')
conc_list <- mapply(function(v1, v2, v3, v4, v5) {
  paste(v1, v2, v3, v4, v5, sep = "_")
}, run_time_str, receptors$long, receptors$lati, receptors$zagl, receptors$obid) 
simulation_id <- as.list(conc_list) 

# Meteorological data input
met_path           <- '/scratch/07351/tg866507/wrfghg/arlout/'
